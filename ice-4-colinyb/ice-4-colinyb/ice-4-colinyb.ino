...
void loop() {
  ...
  if (millis() - timerOne > 10000) {
    ...
    String legoBatmanIronyLevel = "already asleep";
    sprintf(message, "{\"tirednesslevel\":\"%s%\"}", legoBatmanIronyLevel.c_str()); // %d is used for an int
    mqtt.publish("theTopic/tirednesslevel", message);
    timerOne = millis();
  }
  ...
  if (millis() - timerTwo > 15000) {
    ...
    mqtt.publish("theTopic/colinyb/tempHum", message);
    timerTwo = millis();
  }
  ...
  if (millis() - timerThree > 23000) {
    ...
    mqtt.publish("theTopic/colinyb/switch", message);
    timerThree = millis();
  }
}//end Loop
...
void callback(char* topic, byte* payload, unsigned int length) {
  ...
  if (strcmp(topic, "theTopic/LBIL") == 0) { //heres a comment - this code listens for theTopic/LBIL messages
    Serial.println("A message from Batman . . .");
  }

  else if (strcmp(topic, "theTopic/tempHum") == 0) { //heres a comment - this code listens for theTopic/tempHum messages
    Serial.println("Some weather info has arrived . . .");
  }

  else if (strcmp(topic, "theTopic/switch") == 0) { //heres a comment - this code listens for theTopic/switch messages
